...
namespace TeamsLeague.BLL.Services
{
    public class MemberService : IMemberService
    {
        ...
        public MemberModel UpdateMember(MemberModel memberModel)
        {
            ...
            member.Mobility = memberModel.Mobility;
            ...
            //REFRESH MATCHHISTORY
            foreach (var seatModel in memberModel.SeatsHistory)
            {
                var seat = member.SeatsHistory.FirstOrDefault(s => s.Id == seatModel.Id);

                if (seat is null)
                {
                    seat = new();

                    member.SeatsHistory.Add(seat);
                    Context.MatchSeats.Add(seat);
                }

                seat.GainedRankPoints = seatModel.GainedRankPoints;
                seat.GainedExperience = seatModel.GainedExperience;
                seat.GainedGold = seatModel.GainedGold;
                seat.Position = seatModel.Position;
                seat.Side = seatModel.Side;
                seat.Member = member;
                seat.MatchId = seatModel.Match.Id;

                foreach (var killM in seatModel.Kills)
                {
                    var kill = seat.Kills.FirstOrDefault(k => k.Id == killM.Id);

                    if (kill is null)
                    {
                        kill = new()
                        {
                            EventId = killM.Event.Id,
                        };

                        seat.Kills.Add(kill);
                        Context.MatchEventKills.Add(kill);
                    }

                    kill.DamageDone = killM.DamageDone;
                    kill.DamageTaken = killM.DamageTaken;
                    kill.DamageBlocked = killM.DamageBlocked;
                    kill.HealingDone = killM.HealingDone;
                    kill.ShieldsDone = killM.ShieldsDone;
                    kill.ControlTime = killM.ControlTime;
                    kill.ControlTaken = killM.ControlTaken;
                    kill.SlowsDone = killM.SlowsDone;
                    kill.SlowsTaken = killM.SlowsTaken;
                }
                foreach (var deathM in seatModel.Dead)
                {
                    var death = seat.Dead.FirstOrDefault(d => d.Id == deathM.Id);

                    if (death is null)
                    {
                        death = new()
                        {
                            EventId = deathM.Event.Id,
                        };

                        seat.Dead.Add(death);
                        Context.MatchEventDeaths.Add(death);
                    }

                    death.DamageDone = deathM.DamageDone;
                    death.DamageTaken = deathM.DamageTaken;
                    death.DamageBlocked = deathM.DamageBlocked;
                    death.HealingDone = deathM.HealingDone;
                    death.ShieldsDone = deathM.ShieldsDone;
                    death.ControlTime = deathM.ControlTime;
                    death.ControlTaken = deathM.ControlTaken;
                    death.SlowsDone = deathM.SlowsDone;
                    death.SlowsTaken = deathM.SlowsTaken;
                }
                foreach (var assistM in seatModel.Assists)
                {
                    var assist = seat.Assists.FirstOrDefault(a => a.Id == assistM.Id);

                    if (assist is null)
                    {
                        assist = new()
                        {
                            EventId = assistM.Event.Id,
                        };

                        seat.Assists.Add(assist);
                        Context.MatchEventAssists.Add(assist);
                    }

                    assist.DamageDone = assistM.DamageDone;
                    assist.DamageTaken = assistM.DamageTaken;
                    assist.DamageBlocked = assistM.DamageBlocked;
                    assist.HealingDone = assistM.HealingDone;
                    assist.ShieldsDone = assistM.ShieldsDone;
                    assist.ControlTime = assistM.ControlTime;
                    assist.ControlTaken = assistM.ControlTaken;
                    assist.SlowsDone = assistM.SlowsDone;
                    assist.SlowsTaken = assistM.SlowsTaken;
                }
                foreach (var destructionM in seatModel.Destructions)
                {
                    var destruction = seat.Destructions.FirstOrDefault(d => d.Id == destructionM.Id);

                    if (destruction is null)
                    {
                        destruction = new();

                        seat.Destructions.Add(destruction);
                        Context.MatchEvents.Add(destruction);
                    }
                }
            }
